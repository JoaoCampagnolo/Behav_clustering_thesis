...
def plot_drosophila_2d(pts=None, draw_joints=None, img=None, colors=None, thickness=None,
                       draw_limbs=None, circle_color=None, draw_order=None):
    if colors is None:
...
def plot_drosophila_3d(ax_3d, points3d, cam_id, bones=skeleton.bones, ang=None, draw_joints=None):
    if draw_joints is None:
        draw_joints = np.arange(skeleton.num_joints)
    points3d = np.array(points3d)
    # colors_tmp = ["red", "blue", "green", "orange", "orange", "orange", "cyan", "purple", "orange", "orange"]
    colors_tmp = ['#%02x%02x%02x' % c for c in skeleton.colors]

    colors = []
    for i in range(skeleton.num_joints):
        colors.append(colors_tmp[skeleton.get_limb_id(i)])
    colors = np.array(colors)
    limb_idx_list = np.arange(0, skeleton.num_limbs)
    joint_idx_list = np.arange(0, skeleton.num_joints)
    ...
    points3d_draw = points3d[draw_joints, :]
    colors_draw = colors[draw_joints]
    ax_3d.scatter3D(points3d_draw[:, 0], points3d_draw[:, 1], points3d_draw[:, 2],
                    c=colors_draw, s=100)
